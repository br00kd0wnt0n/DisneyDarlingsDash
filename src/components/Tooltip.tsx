...
export function Tooltip({ content, children, position = 'top' }: TooltipProps) {
  ...
  // Use right-aligned positioning to prevent overflow on right sidebar
  ...
  return (
    ...
    >
      ...
      {isVisible && (
        <div
          className={`absolute z-50 px-3 py-2 text-sm bg-gray-900 text-white rounded-lg shadow-lg max-w-[200px] ${positionClasses[position]}`}
        >
          {content}
          <div
            className={`absolute w-2 h-2 bg-gray-900 transform rotate-45 ${
              position === 'top' ? 'top-full right-4 -mt-1' :
              position === 'bottom' ? 'bottom-full right-4 -mb-1' :
